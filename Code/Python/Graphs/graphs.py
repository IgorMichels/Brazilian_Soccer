...
if __name__ == '__main__':
    ...
    print(np.sum((home_counts - expected_home)**2 / expected_home), chi2.isf(0.05, len(home) - 2))
    print(np.sum((away_counts - expected_away)**2 / expected_away), chi2.isf(0.05, len(away) - 2))
    
    home_counts = home_counts / np.sum(home_counts)
    away_counts = away_counts / np.sum(away_counts)
    plt.bar(home, home_counts, label = 'Gols - mandante', alpha = 0.7)
    plt.bar(away, away_counts, label = 'Gols - visitante', alpha = 0.7)
    plt.plot(home, home_pois, label = f'Poisson({home_avg:.2f})')
    plt.plot(away, away_pois, label = f'Poisson({away_avg:.2f})')
    plt.legend()
    plt.show()
