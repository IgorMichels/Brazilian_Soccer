...
for player in players:
    ...
    plt.axvline(ataque_m_avg, color = colors[0], linestyle = 'dashed', linewidth = 1, label = f'Ataque - mandante (média): {ataque_m_avg:.2f}')
    plt.axvline(ataque_v_avg, color = colors[1], linestyle = 'dashed', linewidth = 1, label = f'Ataque - visitante (média): {ataque_v_avg:.2f}')
    plt.title(name)
    plt.legend()
    ...
    plt.axvline(defesa_m_avg, color = colors[0], linestyle = 'dashed', linewidth = 1, label = f'Defesa - mandante (média): {defesa_m_avg:.2f}')
    plt.axvline(defesa_v_avg, color = colors[1], linestyle = 'dashed', linewidth = 1, label = f'Defesa - visitante (média): {defesa_v_avg:.2f}')
    plt.title(name)
    plt.legend()
